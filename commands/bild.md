...
**Erfolgskontrolle:** Vor dem Lauf `touch /tmp/img-marker-$$`, danach NUR Bilder öffnen, die `find -newer` als der Marker sind — nie blind „das neueste PNG" (öffnet sonst alte Testbilder). Codex-Output immer in eine Log-Datei schreiben (nie `>/dev/null 2>&1`) und bei KEIN_BILD die letzten Zeilen ausgeben.
